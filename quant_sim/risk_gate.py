...
class RiskGate:
    ...
    def max_positions(self) -> int:
        try:
            n = int(self._rg.get("max_positions", 3))
    ...
    def buy_blocked_reason(self, decision: dict[str, Any] | None, positions_count: int) -> str | None:
        ...
        if self.require_notebooklm_for_buy():
            src = (d.get("decision_source") or d.get("knowledge_source") or "").strip().lower()
            if src != "notebooklm":
                return (
                    "require_notebooklm_for_buy：开仓必须走 NotebookLM 主路径"
                    "（decision_source/knowledge_source 须为 notebooklm），当前来源非 notebooklm，禁止开仓。"
                )
        ...
        ks = (d.get("knowledge_source") or "").strip().lower()
        if ks == "unavailable":
